...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
  ...
  <xsl:template match="container">
    <page>
      ...
      <content>
        <xsl:call-template name="conflict"/>
      </content>
    </page>
  </xsl:template>

  <xsl:template name="conflict">
    ...
    <div class="conflict-resolution">
      ...
      <form action="{$pageURI}" method="post">
        <select id="resolution" name="resolution">
          ...
          <option value="DSY_OVERWRITE">Override with daisy values</option>
          <option value="CONFLICT_DSY_RULES">Permanently override with daisy values</option>
        </select>
        <br/>
