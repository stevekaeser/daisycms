...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="taskMonitorCallbacks">
    <script language="javascript">
      ...
      function taskFinished() {
        ...
        window.location="<xsl:value-of select="$continuationPath"/>/review";
      }
  ...
  <xsl:template name="content">
    <h1><i18n:text key="serp.replace.progress.title"/></h1>

    <div id="dsy-serp-task-busy">
      <xsl:if test="$task/@finishedAt">
        <xsl:attribute name="style">display:none</xsl:attribute>
      </xsl:if>
      <p><i18n:text key="serp.replace.patience"/></p>
    ...
    <div id="dsy-serp-task-finished">
      ...
      <p>
        <i18n:text key="serp.replace.finished.redirect-to"/>
        <a href="{$continuationPath}/review"><i18n:text key="serp.replace.finished.review-page"/></a>.
      </p>
    </div>
  </xsl:template>
