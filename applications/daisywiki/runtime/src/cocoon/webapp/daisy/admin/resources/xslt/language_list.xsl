...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="page">
    <page>
      ...
      <pageTitle>Daisy: Language Administration</pageTitle>
  ...
  <xsl:template name="content">
    ...
    <ul>
      <li><a href="language/new">Create a new Language</a></li>
    </ul>
    <xsl:apply-templates select="d:languages"/>

    <script type="text/javascript">
      function deleteLanguage(id, name) {
        var confirmed = confirm("Are you sure you want to delete the language \"" + name + "\"?");
        if (confirmed) {
    ...
    <div style="display: none">
      <form name="actionForm" method="POST" action="language">
  ...
  <xsl:template match="d:languages">
    <table class="default">
      <tr>
        <th>
          ID
      ...
      <xsl:call-template name="insertSortedLanguages"/>
    </table>
  </xsl:template>

  <xsl:template name="insertSortedLanguages">
    <xsl:choose>
      <xsl:when test="$sortKey = 'id' and $sortOrder = 'asc'">
        <xsl:apply-templates select="d:language">
    ...
    <tr>
      ...
      <td>
        <xsl:choose>
          ...
          <xsl:otherwise>
            <a href="language/{@id}/edit">view/edit</a> |
            <a href="#" onClick="deleteLanguage('{@id}', '{@name}'); return false;">delete</a>
          </xsl:otherwise>
        </xsl:choose>
      </td>
    </tr>
