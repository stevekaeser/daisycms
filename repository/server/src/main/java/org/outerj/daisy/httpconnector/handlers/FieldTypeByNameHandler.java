...
public class FieldTypeByNameHandler extends AbstractRepositoryRequestHandler {
    public String getPathPattern() {
        return "/schema/fieldTypeByName/*";
    }
    ...
    public void handleRequest(Map matchMap, HttpServletRequest request, HttpServletResponse response, Repository repository, RequestHandlerSupport support) throws Exception {
        ...
        if (request.getMethod().equals(HttpConstants.GET)) {
            FieldType fieldType = repository.getRepositorySchema().getFieldTypeByName(name, true);
            fieldType.getXml().save(response.getOutputStream());
        } else {
