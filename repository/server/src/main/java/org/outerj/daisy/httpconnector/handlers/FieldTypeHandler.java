...
public class FieldTypeHandler extends AbstractRepositoryRequestHandler {
    ...
    public void handleRequest(Map matchMap, HttpServletRequest request, HttpServletResponse response, Repository repository, RequestHandlerSupport support) throws Exception {
        ...
        if (request.getMethod().equals(HttpConstants.GET)) {
            FieldType fieldType = repositorySchema.getFieldTypeById(id, true);
            XmlObject fieldTypeDocument = fieldType.getXml();
            fieldTypeDocument.save(response.getOutputStream());
        } else if (request.getMethod().equals(HttpConstants.POST)) {
            // A POST updates the FieldType
            FieldType fieldType = repositorySchema.getFieldTypeById(id, true);
            ...
            if (fieldType.getUpdateCount() != fieldTypeXml.getUpdateCount())
            ...
            repositorySchema.deleteFieldType(id);
        } else {
