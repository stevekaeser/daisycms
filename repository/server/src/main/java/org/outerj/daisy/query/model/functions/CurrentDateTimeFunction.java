...
public class CurrentDateTimeFunction extends AbstractFunction {
    public static final String NAME = "CurrentDateTime";
    ...
    public int bindValueExpr(PreparedStatement stmt, int bindPos, QValueType valueType, EvaluationInfo evaluationInfo) throws SQLException, QueryException {
        stmt.setTimestamp(bindPos, new Timestamp(((Date)evaluate(null, null, evaluationInfo)).getTime()));
        return ++bindPos;
    }
    ...
    protected int[] parseExpression(String expression) throws QueryException {
        Pattern pattern = Pattern.compile("([+-])\\s*([0-9]+)\\s*(days|weeks|months|years|hours|minutes|seconds)");
        Matcher matcher = pattern.matcher(expression);
        if (!matcher.matches())
            throw new QueryException("Invalid relative datetime expression: \"" + expression + "\"");
        ...
        else if (fieldName.equals("minutes"))
            field = Calendar.MINUTE;
        else if (fieldName.equals("seconds"))
            field = Calendar.SECOND;
        else
            throw new RuntimeException("Unexpected value for datetime field name in " + NAME + " function: " + fieldName);

        return new int[] {value, field};
    }

    public ValueExpr clone() {
        CurrentDateTimeFunction clone = new CurrentDateTimeFunction();
