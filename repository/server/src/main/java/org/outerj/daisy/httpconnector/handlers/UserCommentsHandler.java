...
public class UserCommentsHandler extends AbstractRepositoryRequestHandler {
    ...
    public void handleRequest(Map matchMap, HttpServletRequest request, HttpServletResponse response, Repository repository, RequestHandlerSupport support) throws Exception {
        if (request.getMethod().equals(HttpConstants.GET)) {
            ...
            if (visibilityParam != null)
                visibility = CommentVisibility.fromString(visibilityParam);

            response.setContentType("text/xml");
            Comments comments;
            if (visibility != null)
                comments = repository.getCommentManager().getComments(visibility);
            else
                comments = repository.getCommentManager().getComments();

            comments.getXml().save(response.getOutputStream());
        } else {
