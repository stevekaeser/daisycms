...
public class AvailableVariantsHandler extends AbstractRepositoryRequestHandler {
    public String getPathPattern() {
        return "/document/*/availableVariants";
    }
    ...
    public void handleRequest(Map matchMap, HttpServletRequest request, HttpServletResponse response, Repository repository, RequestHandlerSupport support) throws Exception {
        ...
        if (request.getMethod().equals(HttpConstants.GET)) {
            AvailableVariants availableVariants = repository.getAvailableVariants(documentId);
            availableVariants.getXml().save(response.getOutputStream());
        } else {
