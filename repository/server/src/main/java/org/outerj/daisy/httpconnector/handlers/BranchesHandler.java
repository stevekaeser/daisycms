...
public class BranchesHandler extends AbstractRepositoryRequestHandler {
    ...
    public void handleRequest(Map matchMap, HttpServletRequest request, HttpServletResponse response, Repository repository, RequestHandlerSupport support) throws Exception {
        ...
        if (request.getMethod().equals(HttpConstants.POST)) {
            // A POST creates a new Branch and returns the XML representation of the newly created Branch
            ...
            Branch branch = variantManager.createBranch(branchXml.getName());
            ...
            // A GET returns the list of all Branches as XML
            Branches branches = variantManager.getAllBranches(true);
            branches.getXml().save(response.getOutputStream());
        } else {
