...
public class NamespacesHandler extends AbstractRepositoryRequestHandler {
    ...
    public void handleRequest(Map matchMap, HttpServletRequest request, HttpServletResponse response, Repository repository, RequestHandlerSupport support) throws Exception {
        ...
        if (request.getMethod().equals(HttpConstants.GET)) {
            nsManager.getAllNamespaces().getXml().save(response.getOutputStream());
        } else if (request.getMethod().equals(HttpConstants.POST)) {
            String namespaceName = HttpUtil.getStringParam(request, "name");
            String fingerprint = request.getParameter("fingerprint");

            Namespace namespace;
            if (fingerprint == null)
                namespace = nsManager.registerNamespace(namespaceName);
            else
                namespace = nsManager.registerNamespace(namespaceName, fingerprint);
