...
public class DocumentTypeByNameHandler extends AbstractRepositoryRequestHandler {
    public String getPathPattern() {
        return "/schema/documentTypeByName/*";
    }
    ...
    public void handleRequest(Map matchMap, HttpServletRequest request, HttpServletResponse response, Repository repository, RequestHandlerSupport support) throws Exception {
        ...
        if (request.getMethod().equals(HttpConstants.GET)) {
            DocumentType documentType = repository.getRepositorySchema().getDocumentTypeByName(name, true);
            documentType.getXml().save(response.getOutputStream());
        } else {
