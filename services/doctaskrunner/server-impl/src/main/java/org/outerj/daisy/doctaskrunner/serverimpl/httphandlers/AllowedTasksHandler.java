...
import org.outerx.daisy.x10Doctaskrunner.AllowedTasksDocument.AllowedTasks;

public class AllowedTasksHandler extends AbstractDocTaskRequestHandler {
    ...
    public void handleRequest(Map matchMap, HttpServletRequest request, HttpServletResponse response, Repository repository, RequestHandlerSupport support) throws Exception {
        ...
        if (request.getMethod().equals(HttpConstants.GET)) {
            AllowedTasksDocument result = AllowedTasksDocument.Factory.newInstance();
        	AllowedTasks allowedTasks = result.addNewAllowedTasks();
            for (String task: taskManager.getAllowedTasks()) {
            	allowedTasks.addAllowedTask(task);
            }
