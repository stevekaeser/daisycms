...
import org.outerx.daisy.x10.SubscribedDocument;

import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import java.util.Map;

public class DocumentSubscriptionForUserHandler extends AbstractSubscriptionRequestHandler {
    public void handleRequest(Map matchMap, HttpServletRequest request, HttpServletResponse response, Repository repository, RequestHandlerSupport support) throws Exception {
        ...
        if (request.getMethod().equals("GET")) {
            ...
            long userId = HttpUtil.parseId("user", (String)matchMap.get("2"));
            boolean isSubscribed = subscriptionManager.isSubsribed(userId, new VariantKey(documentId, branchId, languageId));
            SubscribedDocument subscribedDocument = SubscribedDocument.Factory.newInstance();
            subscribedDocument.setSubscribed(isSubscribed);
            subscribedDocument.save(response.getOutputStream());
        } else {
