...
public class CollectionSubscriptionsHandler extends AbstractSubscriptionRequestHandler {
    public void handleRequest(Map matchMap, HttpServletRequest request, HttpServletResponse response, Repository repository, RequestHandlerSupport support) throws Exception {
        long collectionId = HttpUtil.parseId("collection", (String)matchMap.get("1"));
        ...
        if (request.getMethod().equals("DELETE")) {
            ...
            subscriptionManager.deleteAllSubscriptionsForCollection(collectionId);
        } else {
