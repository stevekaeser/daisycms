...
public class LdapAuthenticationFactory {
    ...
    public LdapAuthenticationFactory(Configuration configuration, PluginRegistry pluginRegistry) throws Exception {
    ...
    private void configure(Configuration configuration) throws ConfigurationException {
        ...
        for (Configuration schemeConf : schemeConfs) {
            ...
            Configuration[] properties = schemeConf.getChild("environment").getChildren("property");
            Map templateEnvironment = new HashMap();
            for (Configuration property : properties) {
                templateEnvironment.put(property.getAttribute("name"), property.getAttribute("value"));
            }
            UserCreator userCreator = UserCreatorFactory.createUser(schemeConf, name);
            String searchBase = schemeConf.getChild("searchBase").getValue();
            String filter = schemeConf.getChild("filter").getValue("cn=$daisyLogin");
            
            AuthenticationScheme scheme = new LdapAuthenticationScheme(name, description, templateEnvironment, searchBase, filter, userCreator);
