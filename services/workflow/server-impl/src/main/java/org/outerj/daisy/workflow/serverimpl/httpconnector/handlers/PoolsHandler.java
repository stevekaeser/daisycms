...
public class PoolsHandler extends AbstractWorkflowRequestHandler {
    public void handleRequest(Map matchMap, HttpServletRequest request, HttpServletResponse response, Repository repository, RequestHandlerSupport support) throws Exception {
        ...
        if (request.getMethod().equals(HttpConstants.GET)) {
            List<WfPool> pools;
            if (request.getParameter("limitToUser") != null) {
                long userId = HttpUtil.getLongParam(request, "limitToUser");
                pools = poolManager.getPoolsForUser(userId);
            } else {
                pools = poolManager.getPools();
            }
            WfListHelper.getPoolsAsXml(pools).save(response.getOutputStream());
            ...
            WfPool pool = poolManager.createPool(poolDocument.getPool().getName());
