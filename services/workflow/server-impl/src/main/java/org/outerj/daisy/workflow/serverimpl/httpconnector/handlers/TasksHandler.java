...
public class TasksHandler extends AbstractWorkflowRequestHandler {
    public void handleRequest(Map matchMap, HttpServletRequest request, HttpServletResponse response, Repository repository, RequestHandlerSupport support) throws Exception {
        ...
        if (request.getMethod().equals(HttpConstants.GET)) {
            ...
            TasksDocument document = null;
            String select = request.getParameter("select");
            if (select == null) {
                document = WfListHelper.getTasksAsXml(workflowManager.getMyTasks(locale));
            } else if (select.equals("pooled")) {
                document = WfListHelper.getTasksAsXml(workflowManager.getPooledTasks(locale));
            } else {
                HttpUtil.sendCustomError("Invalid value for select parameter: " + select, HttpConstants._400_Bad_Request, response);
            }
