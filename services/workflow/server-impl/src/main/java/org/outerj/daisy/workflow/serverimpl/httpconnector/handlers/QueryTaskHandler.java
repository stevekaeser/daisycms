...
public class QueryTaskHandler extends AbstractQueryHandler {
    XmlObject performQuery(WorkflowManager workflowManager, List<QuerySelectItem> selectItems, QueryConditions queryConditions, List<QueryOrderByItem> orderByItems, int chunkOffset, int chunkLength, Locale locale) throws RepositoryException {
        XmlObject result;
        if (selectItems == null) {
            List<WfTask> tasks = workflowManager.getTasks(queryConditions, orderByItems, chunkOffset, chunkLength, locale);
            result = WfListHelper.getTasksAsXml(tasks);
        } else {
            result = workflowManager.searchTasks(selectItems, queryConditions, orderByItems, chunkOffset, chunkLength, locale);
        }
        return result;
    }
