...
//    	settings.setTooLong(100000*100000);
        ...
        for (RangeDifference d : pdifferences) {
            ...
            if (d.leftStart() > currentIndexLeft) {
            	rightComparator.handlePossibleChangedPart(currentIndexLeft, d.leftStart(), currentIndexRight, d
                        .rightStart(), leftComparator);
            }
            if (d.leftLength() > 0) {
    ...
    private List<RangeDifference> preProcess(
            RangeDifference[] differences, TextNodeComparator leftComparator) {
        ...
        for (int i = 0; i < differences.length; i++) {
            ...
            while(i + 1 < differences.length && differences[i+1].kind()==kind 
            		&& score(leftLength,differences[i+1].leftLength()
            			,rightLength,differences[i+1].rightLength())
