...
public class TextNodeComparator implements IRangeComparator, Iterable<TextNode> {
	...
	public void handlePossibleChangedPart(int leftstart, int leftend, int rightstart,
            int rightend, TextNodeComparator leftComparator) {
    	int i = rightstart;
        int j = leftstart;
	...
	public void markAsDeleted(int start, int end, TextNodeComparator oldComp,
		...
		for (int i = start; i < end; i++) {
			...
			//oldComp is used here because we're going to move its deleted elements
			// to this tree!
		...
		while (deletedNodes.size() > 0) {
			...
			if (nextLeaf != null) {
				...
				nextResult
				.setIndexInLastCommonParent(getBodyNode().getNbChildren());
			}
